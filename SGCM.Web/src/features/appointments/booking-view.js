...
export const ICONS = {
  ...
  professional: '<svg viewBox="0 0 42 42" aria-hidden="true"><circle cx="21" cy="14" r="7"/><path d="M8 34c0-7.5 6-12 13-12s13 4.5 13 12"/></svg>',
...
export function renderConfirmation(ui, { doctorLabel, specialtyLabel, slot }) {
  ...
  confirmation.innerHTML = `<div class="confirmation-success"><div class="confirmation-icon">${icon('success')}</div><h2>¡Cita registrada con éxito!</h2><p>Hemos recibido tu solicitud y te notificaremos cuando sea confirmada.</p></div><section class="appointment-details" aria-labelledby="appointment-details-title"><div class="appointment-details-heading"><span></span><h3 id="appointment-details-title">${icon('calendar')}Detalles de tu cita</h3><span></span></div><dl class="appointment-details-card"><div class="appointment-detail"><div class="appointment-detail-icon">${icon('stethoscope')}</div><dt>Especialidad</dt><dd>${escapeHtml(specialtyLabel)}</dd></div><div class="appointment-detail"><div class="appointment-detail-icon">${icon('professional')}</div><dt>Profesional</dt><dd>${escapeHtml(doctorLabel)}</dd></div><div class="appointment-detail"><div class="appointment-detail-icon">${icon('calendar')}</div><dt>Fecha y hora</dt><dd>${escapeHtml(formatDate(slot))}</dd><time>${icon('clock')}${escapeHtml(formatTime(slot))}</time></div></dl></section><a class="appointment-status-link" href="/mis-citas.html">${icon('appointmentStatus')}<span>Ver estado de mi cita</span></a>`
