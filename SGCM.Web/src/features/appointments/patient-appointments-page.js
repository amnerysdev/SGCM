...
const countTextEl = document.getElementById('appointments-count-text')
...
async function loadAppointments(patientId) {
  try {
    ...
    renderAppointments({ list, countEl, countTextEl }, appointments || [], { ...feedback, reload: () => loadAppointments(patientId) })
  } catch (error) {
